    ...
    for i in tqdm_(range(start_iter, iterations+1)):
        ...
        if arg.pred_step==1:
            ...
            for pred_z_t,ema_z_t in zip(pred_z_t_list,ema_z_t_list):
                if arg.pred_step == 2:
                    predstep_loss_list.append(predstep_2_weight[_iii]*criticion_2(pred_z_t,ema_z_t))
                if arg.pred_step == 3:
                    predstep_loss_list.append(predstep_3_weight[_iii]*criticion_2(pred_z_t,ema_z_t))
                _iii+=1
        ...
        if i % 100 == 0 and rank == 0:
            print(f"Iteration {i}: loss {loss.item()}, loss_fm {loss_fm.item()}, loss_prior {loss_prior:.8f}, loss_sam {loss_sam.item():.8f}, predstep_loss:{predstep_loss_list}")
            ...
            with open(os.path.join(dir, 'log.txt'), 'a') as f:
                f.write(f"Iteration {i}: loss {loss:.8f}, loss_fm {loss_fm:.8f}, loss_prior {loss_prior:.8f}, loss_sam {loss_sam.item():.8f}, lr {optimizer.param_groups[0]['lr']:.4f}, predstep_loss:{predstep_loss_list} \n")
