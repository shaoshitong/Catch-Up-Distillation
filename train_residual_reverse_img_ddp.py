...
def get_args():
    ...
    parser.add_argument('--residual-number',type=int,default=10, help='residual train number')
...
def train_rectified_flow(rank, rectified_flow, forward_model, optimizer, data_loader, iterations, device, start_iter, warmup_steps, dir, learning_rate, independent,
    ...
    for i in tqdm_(range(start_iter, iterations+1)):
        ...
        def get_model_list(re_flow):
            r_model_list = []
            for sub_model in re_flow.model_list:
                r_model_list.append(sub_model.module.state_dict())
            return r_model_list
...
def main(rank: int, world_size: int, arg):
    ...
    for ii in range(start_residual_number, arg.residual_number):
        ...
        for jj in range(len(model_list)):
            model_list[jj] = model_list[jj].to(device)
        ...
        for jj in range(len(model_list)):
            model_list[jj] = DDP(model_list[jj], device_ids=[rank])
        if forward_model is not None:
